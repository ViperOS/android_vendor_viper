 ...
 */
package com.viper.themechooser.adapters;
...
public class PreviewPagerAdapter extends PagerAdapter {
    ...
    public PreviewPagerAdapter(Context context, Configuration config, int[] previewSampleResIds) {
        boolean isLayoutRtl = config.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL;

        mPreviewFrames = new FrameLayout[previewSampleResIds.length];
        mViewStubInflated = new boolean[previewSampleResIds.length];

        for (int i = 0; i < previewSampleResIds.length; ++i) {
            int p = isLayoutRtl ? previewSampleResIds.length - 1 - i : i;
    ...
    public void setPreviewLayer(int newLayerIndex, int currentLayerIndex, int currentFrameIndex,
        ...
        for (FrameLayout previewFrame : mPreviewFrames) {
            if (currentLayerIndex >= 0) {
                ...
                if (mViewStubInflated[currentFrameIndex]) {
