function __print_viper_functions_help() {
cat <<EOF
...
- vipergerrit:   A Git wrapper that fetches/pushes patch from/to ViperOS Gerrit Review.
- viperrebase:   Rebase a Gerrit change and push it again.
- viperremote:   Add git remote for ViperOS Gerrit Review.
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if (adb shell getprop ro.viper.device | grep -q "$LINEAGE_BUILD"); then
...
function viperremote()
{
    ...
    git remote rm viper 2> /dev/null
    ...
    local LINEAGE_USER=$(git config --get review.review.viperos.org.username)
    if [ -z "$LINEAGE_USER" ]
    then
        git remote add viper ssh://review.viperos.org:29418/$PFX$PROJECT
    else
        git remote add viper ssh://$LINEAGE_USER@review.viperos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'viper' created"
}
...
function installboot()
{
    ...
    if (adb shell getprop ro.viper.device | grep -q "$LINEAGE_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        viperremote
        git push viper HEAD:refs/heads/'$1'
    fi
    '
}

function vipergerrit() {
    ...
    local user=`git config --get review.review.viperos.org.username`
...
function viperrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      viperrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.viperos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.viper.device | grep -q "$LINEAGE_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
