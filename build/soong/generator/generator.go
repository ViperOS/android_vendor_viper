...
func init() {
	android.RegisterModuleType("viper_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = viperExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = viperExpandVariables(ctx, dep_file)
	...
	cmd := viperExpandVariables(ctx, String(g.properties.Cmd))
